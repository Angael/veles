...
const getDashboard = createServerFn({ method: 'GET' }).handler(async () => {
  ...
  const src = await createCompositeComponent<{
    renderChart?: (args: { series: Array<{ x: number; y: number }> }) => ReactNode;
  }>((props) => (
    <section>
...
function DashboardPage() {
  ...
  return (
    <CompositeComponent
      src={Dashboard.src}
      renderChart={({ series }) => <ResponsiveChart data={series} width={width} />}
    />
  );
}

// Replace with your real chart component
function ResponsiveChart(props: { data: Array<{ x: number; y: number }>; width: number }) {

...
function PostPage() {
  ...
  return (
    <CompositeComponent
      src={src}
      renderActions={({ postId, authorId }) => <PostActions postId={postId} authorId={authorId} />}
    >
      <Comments />
    </CompositeComponent>
  );
  ...
  .handler(async ({ data }) => {
    ...
    const src = await createCompositeComponent<{
      ...
      <section className='product-card'>
        <h2>{product.name}</h2>
        {props.AddToCart ? <props.AddToCart productId={product.id} price={product.price} /> : null}
      </section>
    ));

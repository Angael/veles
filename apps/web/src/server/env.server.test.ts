...
import type { getServerEnv as GetServerEnv } from './env.server';

let getServerEnv: typeof GetServerEnv;
...
describe('getServerEnv', () => {
  beforeEach(async () => {
    ...
    vi.resetModules();
    // Dynamic import is intentional so each test exercises a fresh module after resetModules.
    ({ getServerEnv } = await import('./env.server'));
  });
